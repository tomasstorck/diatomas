...
def ConfigAS(swapColours=False):
    cell0M = bpy.data.materials['cell0']
    cell1M = bpy.data.materials['cell1']
    cell2M = bpy.data.materials['cell2']
    cellMaterial = [cell1M, cell2M, cell2M, cell2M, cell0M, cell1M]         # Java code sets cellType 4 as fil former (red), 0 or 5 as floc former (yellow)
    return cellMaterial
    
def ConfigAOM():
    cellAnmeM = bpy.data.materials['cellAnme']
    cellDssM = bpy.data.materials['cellAnme']
    cellMaterial = [cellAnmeM, cellDssM, cellAnmeM, cellDssM, cellAnmeM, cellDssM]  # Java code sets even as ANME, odd as DSS (and this code is 0-based)
    return cellMaterial
...
def DefineMaterials():
    ...
    cell2M.diffuse_color = (0.0, 0.7, 1.0)                      # Medium-bright blue/E. coli gen 3
    cell2M.diffuse_intensity = 0.7
    cell2M.specular_color = (0.0, 1.0, 1.0)
...
def SetupCameraTop(distance=None, Lx=20, Ly=20, Lz=20, camWidth=1920, camHeight=1080):                  # Add camera to the scene, for top view
    if distance is None:
        distance = max([camWidth/camHeight*Ly+20, camWidth/camHeight*Lz+20, Lx+20])
...
def DeleteCamerasExtra():                                                # Delete only the side and top camera
    for cam in ['CameraSide', 'CameraTop']:
        obj = bpy.data.objects[cam]
        bpy.data.scenes[0].objects.unlink(obj)
        bpy.data.objects.remove(obj)        

def DisableCameraPersp():
    bpy.data.objects['CameraPersp'].hide_render = True
    bpy.data.objects['PointCamPersp'].hide_render=True


###############################################################################

def SetupXYZLegend(location=(0.0, 0.0, 0.0), fontSize=1):                                   # The three arrows at the origin, showing which direction is X, Y, Z
    ...
    for ax,locCyl,locCone,rot in zip(['X', 'Y', 'Z'], \
        ...
        bpy.context.object.active_material = inkM
    # Create text
    bpy.ops.object.text_add(location=np.add((2, -fontSize*5.0*0.5, 0), location))
    xText = bpy.context.object
    xText.name = 'legendX'
    xText.data.body = 'x'
    bpy.ops.object.text_add(location=np.add((-fontSize*5.0*0.5, 2, 0), location))
...
def SetupPlanes(drawPlaneZ=True, drawPlaneGrid=(False, True, True), Lx=20, Ly=20, Lz=20, radiusZPlane=None, stepSize=10):
    whiteM = bpy.data.materials['white']    
...
def SetupTicks(drawTicks = (True, True, True), Lx = 20, Ly = 20, Lz = 20, fontSize=1.0, stepSize=10):
    inkM = bpy.data.materials['ink']
