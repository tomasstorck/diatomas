...
###############################################################################
#%% Import and normalise model and cell specs
argv = sys.argv[sys.argv.index("--")+1:]                    # Get all arguments after -- (Blender won't touch these)
matFile = argv[0];
overwriteDict = {}

model = scipy.io.loadmat(matFile, chars_as_strings=True, mat_dtype=False, squeeze_me=True, struct_as_record=False)['model']

# See if we need to overwrite any settings, either in model or generated by render.py
modelFields = dir(model)
for key in overwriteDict.keys():
    if key in modelFields:
        setattr(model, key, overwriteDict[key])
...
#%% Create camera
camPos = np.array([-12.6215, 42.59335, 27.23742])
camRot = np.array([64.727, 0, 195.954])
...
bpy.context.scene.render.resolution_percentage = 50         # Allows for quick scaling
"""
...
for ax,locCyl,locCone,rot in zip(['X', 'Y', 'Z'], \
      [(Lx/2-axLegCylH/2, Ly/2, 0),               (Lx/2, Ly/2-axLegCylH/2, 0),              (Lx/2, Ly/2, axLegCylH/2)],  \
      [(Lx/2-axLegCylH-axLegConeH/2, Ly/2, 0),    (Lx/2, Ly/2-axLegCylH-axLegConeH/2, 0),   (Lx/2, Ly/2, axLegCylH+axLegConeH/2)],  \
      [(0, -pi/2, 0), (pi/2, 0, 0), (0, 0, 0)]):
    bpy.ops.mesh.primitive_cylinder_add(radius=0.1, depth=axLegCylH, location=locCyl, rotation=rot)
    bpy.ops.object.shade_smooth()
    bpy.context.object.name = 'cyl'+ax
    bpy.context.object.active_material = inkM
    bpy.ops.mesh.primitive_cone_add(radius1=0.2, depth=axLegConeH, location=locCone, rotation=rot)
    bpy.ops.object.shade_smooth()
    bpy.context.object.name = 'cone'+ax
    bpy.context.object.active_material = inkM
# Create text
bpy.ops.object.text_add(location=(Lx/2-2, Ly/2+1.5, 0))
xText = bpy.context.object
xText.data.body = 'x'
bpy.ops.object.text_add(location=(Lx/2+1.5, Ly/2-2, 0))
yText = bpy.context.object
yText.data.body = 'y'
bpy.ops.object.text_add(location=(Lx/2+1.5, Ly/2+1.5, 0))
...
if model.normalForce:
    ...
    bpy.ops.mesh.primitive_plane_add(radius=Lx/2, location=(0, 0, -0.1))  # Plane to project shadows on
    ...
    yPlaneGrid.location[1] = -Ly/2
    yPlaneGrid.location[2] = Lz/2
    ...
    for p in np.arange(0,Lx+1,10):
        tickList = []
        if p > 0:
            bpy.ops.object.text_add(location=(Lx/2-p, Ly/2+1.5, 0))
            xTick = bpy.context.object
            xTick.rotation_euler[2] = pi
            xTick.data.body = str(int(p))    
            tickList.append(xTick)
        if p > 0 and p < Ly:
            bpy.ops.object.text_add(location=(Lx/2+1.5, Ly/2-p, 0))
            yTick = bpy.context.object
            yTick.data.body = str(int(p))
            yTick.rotation_euler[2] = pi
            tickList.append(yTick)
        if True:
            bpy.ops.object.text_add(location=(Lx/2+1.5, -Ly/2, p))
            zTick = bpy.context.object
            zTick.data.body = str(int(p))
            zTick.rotation_euler[0] = 0.5*pi
            zTick.rotation_euler[2] = 1*pi
...
# Unselect everything, get ready for playing
bpy.ops.object.select_all(action='DESELECT')