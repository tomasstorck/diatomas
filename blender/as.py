...
for iStick,stick in enumerate(model.stickSpringArray[:,0]):
    cell0 = model.cellArray[model.ballArray[stick.ballArray[0,0],0].cell[0,0],0]
    cell1 = model.cellArray[model.ballArray[stick.ballArray[1,0],0].cell[0,0],0]
#    if cell0.type[0,0] == cell1.type[0,0]:
#        continue                                # Computational burden: render only sticking links between different cell types
    pos = np.empty([2,3])
