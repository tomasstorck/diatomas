...
dirPath = ['/home/tomas/documenten/modelling/diatomas_symlink/results/ecoli_noanchor_dlvo_newModel2']
iterModDiv = 5 
...
for iFil,fil in enumerate(model.filSpringArray[:,0]):
    ...
    for ii,iBall in enumerate(fil.ballArray):
        ...
        pos[ii,:]   = ball.pos[:,0] * 1e6
    filG = model.CreateSpring(pos, 0.1, filM.name)
...
for iAnchor,anchor in enumerate(model.anchorSpringArray[:,0]):
    ...
    ball = model.ballArray[int(iBall),0]
    pos   = ball.pos[:,0] * 1e6
    anchorG = model.CreateSpring(np.concatenate([[pos, [pos[0],pos[1],0.0]]], 0), 0.1, anchorM.name)
